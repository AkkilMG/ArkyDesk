...
export default function GuestCreate({ showHeader = true }: { showHeader?: boolean }) {
  ...
  return (
    <div className={showHeader ? "max-w-2xl mx-auto bg-white p-6 rounded-lg shadow" : "w-full"}>
      ...
      {error && <div className="mb-3 p-2 text-sm bg-red-50 border border-red-200 text-red-600 rounded">{error}</div>}
      {success && <div className="mb-3 p-2 text-sm bg-green-50 border border-green-200 text-green-600 rounded">{success}</div>}
      {guestLoginUrl && (
      ...
      <form onSubmit={submit} className="space-y-3">
        <div className="grid grid-cols-2 gap-3">
          <input value={name} onChange={e => setName(e.target.value)} placeholder="Full name" className="border border-gray-300 p-2 rounded-lg focus:ring-2 focus:ring-blue-300 focus:outline-none" />
          <input value={email} onChange={e => setEmail(e.target.value)} placeholder="Email" type="email" className="border border-gray-300 p-2 rounded-lg focus:ring-2 focus:ring-blue-300 focus:outline-none" />
        </div>
        <input value={subject} onChange={e => setSubject(e.target.value)} placeholder="Subject" className="w-full border border-gray-300 p-2 rounded-lg focus:ring-2 focus:ring-blue-300 focus:outline-none" />
        <select value={problem} onChange={e => setProblem(e.target.value)} className="w-full border border-gray-300 p-2 rounded-lg focus:ring-2 focus:ring-blue-300 focus:outline-none">
          <option value="">Select problem category</option>
          <option value="crash">Application Crash</option>
          <option value="copyright">Copyright Issues</option>
        </select>
        <textarea value={description} onChange={e => setDescription(e.target.value)} rows={5} placeholder="Describe the issue in detail..." className="w-full border border-gray-300 p-2 rounded-lg focus:ring-2 focus:ring-blue-300 focus:outline-none resize-none"></textarea>
        ...
        <div className="flex justify-end gap-2">
          <button disabled={loading || sendingLink} type="submit" className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:bg-gray-400 disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center gap-2 transition-colors">
            {loading || sendingLink ? (
              <>
